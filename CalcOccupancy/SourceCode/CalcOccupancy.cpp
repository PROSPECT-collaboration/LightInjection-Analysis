...
void TrigFilter(string PC_Output, string Output_Dir, string title, CalcOccupancy Occu) {
  ...
  //plotting variables 
  int nch = 51;               //set of PMT of interest
>>>>>>> origin/master
...
=======
  for (int j = 0; j < nch; j++) {
    PMT[j] = j;  //Do j+50, j+100, etc to shift the ch by 50.
    ostringstream areahis, nhis;
    areahis << "PMT_" << PMT[j] << "_PIA";
    nhis << "his_" << j;    string areahisname = areahis.str();
    string nhisname = nhis.str();    his[j] = new TH1F(nhisname.c_str(), areahisname.c_str(), 100, 0, 300);
>>>>>>> origin/master
  }
  //EXTRACT Area
...
=======
  for (int l =0; l < nch; l++) {
    ...
    PLT_PMT[l] = l;
    PLT1_OccuV[l] = Occu.CalcOccu(Area_Total[l]/counter[l]);
    PLT2_OccuV[l] = Occu.CalcOccu(Area_Total[l]/100000);        
    his[l] -> Write();
    cout << "Occu per pulse seen: " <<  PLT1_OccuV[l] << setw(10) <<  "  Occu per pulse sent: " << PLT2_OccuV[l] << endl;
>>>>>>> origin/master
  }
...
=======
  g1_1->GetYaxis()->SetRangeUser(-0.0, 65.0);    //change y axis window
>>>>>>> origin/master
...
=======
  g2_1->SetTitle(PulseSeenName.c_str());
  g2_1->GetXaxis()->SetTitle("Ch");
  g2_1->GetYaxis()->SetTitle("OccuV");
  g2_1->GetYaxis()->SetRangeUser(0.0, 55.0);      //change Yaxis window
>>>>>>> origin/master
